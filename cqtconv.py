...
def get_subdirs(a_dir):
	...
	return [name for name in os.listdir(a_dir) if (os.path.isdir(os.path.join(a_dir, name)) and not (name.startswith('.')))]
...
def logfmap(I, L, H):
	...
	for i in range(0, opr) :
		...
		# i.e. i = max -> bin = topbin, i = max-1 -> bin = topbin-1, but general form is bin = A exp (i/B)
